...
// seedDispatcherUser inserts a minimal identity.household + identity.member +
// profile row directly, mirroring adapter's own notifyFixture.seedUser —
// this package has no reason to import the identity bounded context's own
// repository just to seed a fixture row.
func seedDispatcherUser(t *testing.T, pool *pgxpool.Pool) identity.UserID {
	...
	if _, err := pool.Exec(ctx, memberQ, id.String(), householdID.String(), email); err != nil {
		t.Fatalf("seed user: %v", err)
	}
	const profileQ = `INSERT INTO profile (member_id, color) VALUES ($1, 'indigo')`
	if _, err := pool.Exec(ctx, profileQ, id.String()); err != nil {
