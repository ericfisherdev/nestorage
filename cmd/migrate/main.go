...
// using Nestorage's embedded migration set, run through nestcore's Runner.
// It reads DATABASE_URL (and, optionally, MIGRATE_DATABASE_URL) via
// nestcore's config loader. New migrations are scaffolded with the `create`
// subcommand.
//
// Usage:
//
...
func run(args []string) error {
	...
	// error rather than failing later in config loading. create needs no
	// database.
	switch command {
	case "create":
		if len(args) != 2 {
		...
		path, err := createMigration(migrate.SourceDir, args[1])
		if err != nil {
			return err
		}
	...
	dbCfg, errs := config.LoadDB()
	errs = append(errs, dbCfg.Validate()...)
	if len(errs) > 0 {
	...
	case "down":
		return runner.Down(ctx, dsn, opts...)
	case "status":
		statuses, err := runner.Status(ctx, dsn, opts...)
		if err != nil {
			return err
		}
		return ncmigrate.WriteStatus(os.Stdout, statuses)
	default: // "reset" (the only remaining validated command)
		return runner.Reset(ctx, dsn, opts...)
	}
}

// migrateSettings resolves the connection the migration tool should use and
// any connection options. It prefers MIGRATE_DATABASE_URL (cfg.MigrateDSN)
// and falls back to DATABASE_URL. The pooler-safe simple protocol is enabled
// only when migrations would otherwise run through the Supabase transaction
// pooler — i.e. no dedicated migrate DSN was provided and the app DSN
// targets it; pointing MIGRATE_DATABASE_URL at the direct/session connection
// is the preferred path.
func migrateSettings(cfg config.DBConfig) (string, []ncmigrate.Option) {
	dsn := cfg.MigrateDSN
	if dsn == "" {
		dsn = cfg.DSN
	}

	var opts []ncmigrate.Option
	if cfg.MigrateDSN == "" &&
		cfg.Provider == config.DBProviderSupabase &&
		cfg.PoolMode == config.DBPoolModeTransaction {
		opts = append(opts, ncmigrate.PoolerSafe())
	}
	return dsn, opts
}
