...
# Pinned Tailwind standalone CLI (no Node toolchain). The platform asset and
# its checksum are auto-detected from uname; keep the version + checksums in
# sync with the official release sha256sums.txt.
...
# cmd/server's own lifecycle test joins them (NSTR-15); internal/identity is
# Sprint 3's bounded context (NSTR-18); appended to as further bounded
# contexts land (Sprint 4, Bins & Items, adds the next one).
GATED_TEST_PACKAGES := ./internal/platform/db/... ./cmd/migrate/... ./cmd/server/... ./internal/identity/...
...
## test-gated: run the database-gated suites (needs NESTORAGE_TEST_DATABASE_URL)
test-gated:
	@test -n "$(NESTORAGE_TEST_DATABASE_URL)" || \
		{ echo "NESTORAGE_TEST_DATABASE_URL is not set; see docs/testing.md"; exit 1; }
